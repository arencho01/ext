Ext4.define('VetmanagerApp.modules.graphic_reports.controller.graphics.DoctorVisits', {
    extend: 'Ext4.app.Controller'
    , views: [
        'VetmanagerApp.modules.graphic_reports.view.graphics.DoctorVisits'
    ]
    , stores: [
        'VetmanagerApp.modules.graphic_reports.store.DoctorVisits'
    ]
    , mixins: [
    ...
    , refs: [
        {
            ref: 'graphic'
            , selector: 'doctor_visits'
            , autoCreate: true
            , xtype: 'doctor_visits'
        }, {
            ref: 'filterDateButtons'
            , selector: 'doctor_visits panel[name="filter_date_buttons"]'
            , autoCreate: true
        }, {
            ref: 'filterDateRange'
            , selector: 'doctor_visits panel[name="filter_date_range"]'
            , autoCreate: true
        }
    ]
    , init: function() {
        this.control({
            'doctor_visits button[action="date_filter"]': {
                click: this.updateGraphic
            }
            , 'doctor_visits datefield[name="date_from"]': {
                select: this.updateGraphic
                , keyup: this.enterDate
            }
            , 'doctor_visits datefield[name="date_to"]': {
                select: this.updateGraphic
                , keyup: this.enterDate
            }
            , 'doctor_visits button[action="clear_current_filter"]': {
