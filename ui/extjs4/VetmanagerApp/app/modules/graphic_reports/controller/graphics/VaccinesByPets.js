Ext4.define('VetmanagerApp.modules.graphic_reports.controller.graphics.VaccinesByPets', {
    extend: 'Ext4.app.Controller'
    , views: [
        'VetmanagerApp.modules.graphic_reports.view.graphics.VaccinesByPets'
    ]
    , stores: [
        'VetmanagerApp.modules.graphic_reports.store.VaccinesByPets'
    ]
    , mixins: [
    ...
    , refs: [
        {
            ref: 'graphic'
            , selector: 'vaccines_by_pets'
            , autoCreate: true
            , xtype: 'vaccines_by_pets'
        }, {
            ref: 'filterDateButtons'
            , selector: 'vaccines_by_pets panel[name="filter_date_buttons"]'
            , autoCreate: true
        }, {
            ref: 'filterDateRange'
            , selector: 'vaccines_by_pets panel[name="filter_date_range"]'
            , autoCreate: true
        }, {
            ref: 'filterPetType'
            , selector: 'vaccines_by_pets panel[name="filter_pet_type"]'
            , autoCreate: true
        }
    ]
    , init: function() {
        this.control({
            'vaccines_by_pets button[action="date_filter"]': {
                click: this.updateGraphic
            }
            , 'vaccines_by_pets datefield[name="date_from"]': {
                select: this.updateGraphic
                , keyup: this.enterDate
            }
            , 'vaccines_by_pets datefield[name="date_to"]': {
                select: this.updateGraphic
                , keyup: this.enterDate
            }
            , 'vaccines_by_pets combo[name="pet_type"]': {
                select: this.updateGraphic
            }
            , 'vaccines_by_pets button[action="clear_current_filter"]': {
    ...
    , getFilterParams: function() {
        var dateRangePanel = this.getFilterDateRange()
            , petTypePanel = this.getFilterPetType()
        ...
        var petTypeCombo = petTypePanel.query('[name="pet_type"]')[0];
        params.pet_type = petTypeCombo.getValue() * 1;
        params.pet_type_raw = petTypeCombo.getRawValue();
